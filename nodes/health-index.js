module.exports = function (RED) {
    ...
    function HealthIndexNode(config) {
        ...
        const persistence = persistenceHelper.initializeStatePersistence(node, {
            ...
            onStateLoaded: function (state) {
                if (state.healthHistory && state.healthHistory.length > 0) {
                    ...
                    if (debug) {
                        node.debug(
                            "Restored " + node.healthHistory.length + " health history entries from persistence"
                        );
                    }
                    node.status({
                        fill: "green",
                        shape: "dot",
                        text: "Restored (" + node.healthHistory.length + " entries)"
                    });
                }
            },
        ...
        node.on("input", function (msg) {
            ...
            const activeHealthyThreshold =
                cfg.healthyThreshold !== undefined
                    ? clampThreshold(cfg.healthyThreshold, healthyThreshold)
                    : healthyThreshold;
            const activeWarningThreshold =
                cfg.warningThreshold !== undefined
                    ? clampThreshold(cfg.warningThreshold, warningThreshold)
                    : warningThreshold;
            const activeDegradedThreshold =
                cfg.degradedThreshold !== undefined
                    ? clampThreshold(cfg.degradedThreshold, degradedThreshold)
                    : degradedThreshold;
            const activeCriticalThreshold =
                cfg.criticalThreshold !== undefined
                    ? clampThreshold(cfg.criticalThreshold, criticalThreshold)
                    : criticalThreshold;
        ...
        function calculateDynamicWeight(sensorName, sensorInfo, baseWeight) {
            ...
            const value =
                sensorInfo.value !== undefined ? sensorInfo.value : typeof sensorInfo === "number" ? sensorInfo : null;
            ...
            if (method === "weighted" || method === "dynamic") {
                ...
                for (const [sensorName, score] of Object.entries(sensorScores)) {
                    ...
                    const effectiveWeight =
                        method === "dynamic" ? dynamicWeight.effectiveWeight : weights[sensorName] || 1.0;
                ...
                const minScores = Object.values(sensorScores);
                healthIndex = minScores.length > 0 ? Math.min(...minScores) : 100;
            } else if (method === "average") {
                // Simple average
                const scores = Object.values(sensorScores);
                healthIndex = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 100;
